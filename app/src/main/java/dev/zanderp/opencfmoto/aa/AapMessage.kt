// Ported from headunit-revived (AGPLv3): aap/AapMessage.kt
...
open class AapMessage(
    ...
    @JvmOverloads
    constructor(channel: Int, type: Int, proto: Message, buf: ByteArray = ByteArray(size(proto)))
    ...
    override fun toString(): String {
        ...
        sb.append(Channel.name(channel)); sb.append(' ')
        sb.append(MsgType.name(type, channel))
        sb.append(" type: "); sb.append(type)
        sb.append(" flags: "); sb.append(flags)
        sb.append(" size: "); sb.append(size)
        sb.append(" dataOffset: "); sb.append(dataOffset)
        return sb.toString()
    }

    override fun equals(other: Any?): Boolean {
        val msg = other as? AapMessage ?: return false
        if (msg.channel != this.channel) return false
        if (msg.flags != this.flags) return false
        if (msg.type != this.type) return false
        if (msg.size != this.size || msg.data.size < this.size) return false
        if (msg.dataOffset != this.dataOffset) return false
        for (i in 0 until this.size) if (msg.data[i] != this.data[i]) return false
        return true
    }
    ...
    companion object {
        ...
        private fun size(proto: Message): Int = proto.serializedSize + MsgType.SIZE + HEADER_SIZE
